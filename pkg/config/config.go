// Package config defines Podsync's TOML configuration model and loading logic.
// It is shared by the main application and the admin API.
package config
...
// parseConfig unmarshals, applies defaults/env overrides and validates raw TOML.
// path is only used to derive defaults (e.g. the database directory).
